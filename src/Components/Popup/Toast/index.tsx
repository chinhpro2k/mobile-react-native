...
import { getWidth, WIDTH, getFontSize, getLineHeight, HEIGHT } from '@/Config'
...
const getColor = (type: TypeToast) => {
  switch (type) {
    case TypeToast.WARNING:
      return R.colors.yellow200
    case TypeToast.ERROR:
      return R.colors.red102
    default:
...
export const showToast = (toastConfig: ToastConfig) => {
  ...
  const ToastComponent = (
    <View style={styles.container}>
      <View
        style={[styles.bage, { backgroundColor: getColor(toastConfig.type) }]}
      />
      <View style={styles.content}>
        ...
        <FastImage
          source={getImage(toastConfig.type)}
          style={styles.img}
          resizeMode="contain"
        />
      </View>
  ...
  const ToastBug = (
    <View style={getStyle(toastConfig.type)}>
      <IconSVG
        iconName={getIcon(toastConfig.type)}
        width={WIDTH(24)}
        height={WIDTH(24)}
      />
  ...
  if (
    toastConfig.type === TypeToast.SUCCESS_BUG ||
    toastConfig.type === TypeToast.ERROR_BUG
  ) {
    toast = Toast.show(
...
const styles = StyleSheet.create({
  ...
  viewToastSuccess: {
    ...
    backgroundColor: R.colors.white,
    borderBottomLeftRadius: WIDTH(8),
    borderLeftColor: R.colors.green600,
    borderLeftWidth: WIDTH(8),
    borderRadius: WIDTH(8),
    borderTopLeftRadius: WIDTH(8),
    flexDirection: 'row',
